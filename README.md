...
- `/standings season:<year>` - Shows standings with a simple team strength score.
...
- `/team team:<autocomplete> season:<year>` - Shows team snapshot, what they need, who they could shop, and points left on the bench.
- `/player player:<autocomplete> week:<number> season:<year>` - Shows player averages, recent games, projection, player type, and trend.
- `/compare player_a:<autocomplete> player_b:<autocomplete> season:<year>` - Compares two players.
- `/market mode:<mode> team:<autocomplete> season:<year>` - Shows players worth asking about, hot players, steady players, and avoids.
- `/trade a1:<player> b1:<player> ... picks:<pick>` - Quickly checks who wins a trade.
- `/tradefinder team:<autocomplete> need:<need> offer_style:<level>` - Suggests simple trade ideas.

The command list is intentionally compact. Draft, playoff, broad history, and weekly recap screens are intentionally left to Sleeper unless they are needed inside a focused team or player summary. Fantasy HQ focuses on quick coach decisions: team strength, roster needs, player trends, and trade ideas.
